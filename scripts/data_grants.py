...
with open('grants.csv') as csv_file:
...
def remove_empty(in_docs, in_timestamps, in_labels):
    out_docs = []
    out_timestamps = []
    out_labels = []
    ...
    return out_docs, out_timestamps, out_labels

def remove_by_threshold(in_docs, in_timestamps, in_labels, thr):
    out_docs = []
    out_timestamps = []
    out_labels = []
    for ii, doc in enumerate(in_docs):
...
docs_tr, timestamps_tr, labels_tr = remove_empty(docs_tr, timestamps_tr, labels_tr)
docs_ts, timestamps_ts, labels_ts = remove_empty(docs_ts, timestamps_ts, labels_ts)
docs_va, timestamps_va, labels_va = remove_empty(docs_va, timestamps_va, labels_va)

# Remove test documents with length=1
docs_ts, timestamps_ts, labels_ts = remove_by_threshold(docs_ts, timestamps_ts, labels_ts, 1)
...
path_save = './data_grants_' + str(flag_split_by_paragraph) + '/min_df_' + str(min_df) + '/'+ 'labels/'
...
# Save labels alone
with open(path_save + 'labels_tr.pkl', 'wb') as f:
    pickle.dump(labels_tr, f)
with open(path_save + 'labels_ts.pkl', 'wb') as f:
    pickle.dump(labels_ts, f)
with open(path_save + 'labels_va.pkl', 'wb') as f:
    pickle.dump(labels_va, f)

label_time_lst=[[] for t in time_list]

for t,l in zip(timestamps_tr,labels_tr):
    label_time_lst[t].append(l)
for t,l in zip(timestamps_va,labels_va):
    label_time_lst[t].append(l)
for t,l in zip(timestamps_ts,labels_ts):
    label_time_lst[t].append(l)

label_time_lst=[set(l) for l in label_time_lst]
with open(path_save + 'label_time.pkl', 'wb') as f:
    pickle.dump(label_time_lst, f)
