...
parser.add_argument('--batch_size', type=int, default=1024, help='number of documents in a batch for training')
...
parser.add_argument('--div_reg', type=int, default=5, help='topic divergence regularization term')
...
data_file = os.path.join(args.data_path, 'min_df_{}'.format(args.min_df)+'/labels')
...
id2lbl = list(set(train_labels+valid_labels+test_labels))
args.lbl_num = len(id2lbl)
lbl2id = {v:k for k,v in enumerate(id2lbl)}
train_labels=[lbl2id[l] for l in train_labels]
valid_labels=[lbl2id[l] for l in valid_labels]
test_labels=[lbl2id[l] for l in test_labels]
print(args.lbl_num)
## get embeddings 
...
def train(epoch):
    ...
    for idx, ind in enumerate(indices):
        optimizer.zero_grad()
        model.zero_grad()
        data_batch, times_batch, labels_batch = data.get_batch(
            train_tokens, train_counts, ind, args.vocab_size, args.emb_size, temporal=True, clf=True, times=train_times, labels=train_labels)
        ...
        loss, nll, kl_alpha, kl_eta, kl_theta = model(data_batch, normalized_data_batch, times_batch, labels_batch, train_rnn_inp, args.num_docs_train)
        loss.backward()
        if args.clip > 0:
    ...
    with open('nelbo_100ep.txt','a') as f:
