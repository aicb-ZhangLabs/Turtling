...
class Turtling(nn.Module):
    def __init__(self, args, embeddings):
        ...
        self.div_reg=args.div_reg
        self.theta_act = self.get_activation(args.theta_act)
        self.num_labels=args.lbl_num

        self.clf=nn.Linear(self.num_topics,self.num_labels)
    ...
    def get_alpha(self): ## mean field
        ...
        dis_alpha.append(torch.cdist(torch.clone(alphas[0]),torch.clone(alphas[0]),p=2).sum())
        #print(dis_alpha[0].shape,dis_alpha[0])
        #exit()
        ...
        for t in range(1, self.num_times):
            ...
            dis_alpha.append(torch.cdist(torch.clone(alphas[t]),torch.clone(alphas[t]),p=2).sum())
            p_mu_t = alphas[t-1]
            logsigma_p_t = torch.log(self.delta * torch.ones(self.num_topics, self.rho_size).to(device))
            kl_t = self.get_kl(self.mu_q_alpha[:, t, :], self.logsigma_q_alpha[:, t, :], p_mu_t, logsigma_p_t)
            kl_alpha.append(kl_t)
        kl_alpha = torch.stack(kl_alpha).sum()
        dis_alpha = torch.stack(dis_alpha).sum()
        torch.autograd.set_detect_anomaly(True)
        #print(kl_alpha.sum(),dis_alpha)
        return alphas, kl_alpha.sum()-self.div_reg*dis_alpha
    ...
    def forward(self, bows, normalized_bows, times, labels, rnn_inp, num_docs):
        ...
        pred=self.clf(theta)
        pred_loss=self.clfloss(pred,labels)

        nelbo = nll + kl_alpha + kl_eta + kl_theta + pred_loss*10
