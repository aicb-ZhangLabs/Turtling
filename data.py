...
def _fetch_temporal(path, name):
    if name == 'train':
        ...
        label_file = os.path.join(path, 'labels_tr.pkl')
    elif name == 'valid':
        token_file = os.path.join(path, 'bow_va_tokens.mat')
        count_file = os.path.join(path, 'bow_va_counts.mat')
        time_file = os.path.join(path, 'bow_va_timestamps.mat')
        label_file = os.path.join(path, 'labels_va.pkl')
    else:
        token_file = os.path.join(path, 'bow_ts_tokens.mat')
        count_file = os.path.join(path, 'bow_ts_counts.mat')
        time_file = os.path.join(path, 'bow_ts_timestamps.mat')
        label_file = os.path.join(path, 'labels_ts.pkl')
    tokens = scipy.io.loadmat(token_file)['tokens'].squeeze()
    counts = scipy.io.loadmat(count_file)['counts'].squeeze()
    ...
    if name == 'test':
        ...
        tokens_2 = scipy.io.loadmat(token_2_file)['tokens'].squeeze()
        counts_2 = scipy.io.loadmat(count_2_file)['counts'].squeeze()
        return {'tokens': tokens, 'counts': counts, 'times': times, 'labels':labels,
...
def get_data(path, temporal=False):
    ...
    #with open(os.path.join(path, 'label_time.pkl'), 'rb') as f:
...
def get_batch(tokens, counts, ind, vocab_size, emsize=300, temporal=False, clf=False, times=None, labels=None):
    ...
    if clf:
        labels_batch = np.zeros((batch_size, ),dtype=int)
    ...
    if temporal:
        ...
        if clf:
            ...
            return data_batch, times_batch, labels_batch
